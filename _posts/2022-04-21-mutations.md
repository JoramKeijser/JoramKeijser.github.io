...
A direct test of this idea could simultaneously map a fitness landscape and detect mutations before they are removed or spread by selection. A  computational approach could test if mutational bias can evolve in silico. Such experiments might reveal that evolution does what engineers expect it to do, but I think this is unlikely. Instead, the data reveal a nuanced situation in which mutations are neither the directionless force they once were thought to be nor the ruthlessly optimal gradients used by engineers. 

In this blog post, we have considered how genetic mutations contribute to the course of evolution. I might use a future blog post to explore the role of processes above the single gene level, such as [development](https://en.wikipedia.org/wiki/Evolutionary_developmental_biology) and [ecology](https://en.wikipedia.org/wiki/Evolutionary_ecology). 
